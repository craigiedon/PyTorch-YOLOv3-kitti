...
from load_salient_dataset import filter_inp_labels, group_data, count_tru_pos_v_false_neg
from pems import PEMClass, SalientObstacleDataset


def run():
    model = PEMClass(7, 1, 20)
    model.cuda()
    ...
    s_inputs = torch.tensor(s_inputs, dtype=torch.float)[::1, [0]]  # Class label
    ...
    s_inputs = F.one_hot(s_inputs.to(dtype=torch.long), 7).to(dtype=torch.float)
    ...
    one_hot_classes = F.one_hot(torch.arange(0, 7), 7).to(dtype=torch.float).cuda()
